...
::::challenge{id=new-machine title="Exercise: Setting up a new machine"}

You've just been given a new laptop and want to start using Git with GitHub on it. Which of the setup steps from this episode do you need to repeat, and which are already done for your GitHub account?

:::solution

On the **new machine** you need to repeat the local setup: set your identity with `git config --global user.name` / `user.email` (and your editor), then generate a **new SSH key** with `ssh-keygen` and add its **public** half to GitHub.

Your **GitHub account** already exists, so you don't recreate that - but each machine gets its own SSH key, so the new laptop's public key must be added alongside any existing ones.
...
- Configure Git once per machine with `git config --global user.name`, `user.email`, and `core.editor`.
- `--global` applies the setting to every repository for your user; check everything with `git config --list`.
- Authenticate to GitHub with an SSH key pair generated by `ssh-keygen -t ed25519`.
- Add the **public** key (`.pub`) to GitHub; the **private** key never leaves your machine.
