...
::::challenge{id=what-to-track title="Exercise: What belongs in version control?"}
...
3. The `README.md` describing the project
4. A `results.pdf` figure generated by running the scripts
5. A file of passwords used to access a database

:::solution

Track **1** and **3** - source code and documentation are exactly what version control is for.

Leave out **2** (large files that can be re-downloaded bloat the repository - store a script or link that fetches it instead), **4** (generated outputs should be reproducible from the source, so track the code that makes them, not the output), and **5** (never commit secrets - anything committed stays in the history forever).
...
- Version control tracks **changes** to files over time, storing a base version plus each change.
- It provides backup, reproducibility, and a way for multiple people to collaborate without overwriting each other.
- **Git** is a **distributed** system: every clone holds the full history. Subversion is an older centralised alternative.
- **Git** is the tool; **GitHub**/**GitLab** are hosting services built around it - they are not the same thing.
- Graphical clients and IDE integrations are just wrappers around the same command-line Git.
